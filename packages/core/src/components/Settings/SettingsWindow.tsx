...
const PluginsSection: React.FC = () => {
  ...
  const handleInstall = async () => {
    ...
    input.onchange = async (e: Event) => {
      ...
      const dirPath = (firstFile as any).path;
      if (dirPath) {
        const result = await window.electronAPI?.installPlugin?.(dirPath);
        if (result?.success) loadPlugins();
      }
    };
    input.click();
  };
  ...
  return (
    <div>
              ...
              <div key={plugin.name} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {summary && (
                      <p style={{ fontSize: '11px', color: 'var(--accent-color)', marginTop: '4px', marginBottom: 0 }}>
                        {summary}
                      </p>
                    )}
                  </div>
      ...
      <div style={{ marginTop: '16px', display: 'flex', gap: '8px' }}>
        ...
        <button onClick={() => window.electronAPI?.openPluginsFolder?.()} style={{
          display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px',
          borderRadius: '6px', border: '1px solid var(--border-color)',
          backgroundColor: 'var(--bg-secondary)', color: 'var(--text-primary)',
          cursor: 'pointer', fontSize: '13px',
        ...
        {plugins.length > 0 && (
          <button onClick={() => window.electronAPI?.reloadMain?.()} style={{
            display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px',
            borderRadius: '6px', border: '1px solid var(--border-color)',
            backgroundColor: 'var(--bg-secondary)', color: 'var(--text-primary)',
            cursor: 'pointer', fontSize: '13px',
